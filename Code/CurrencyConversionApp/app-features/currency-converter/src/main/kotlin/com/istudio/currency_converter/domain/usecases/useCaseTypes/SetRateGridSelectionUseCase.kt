...
) : Action<GridSelectionInput, Result<List<CurrencyAndRates>>>() {
    override suspend fun action(input: GridSelectionInput): Result<List<CurrencyAndRates>> = withContext(context = dispatcher) {
        try {
            input.data.forEachIndexed { index, ratesEntity ->
                ratesEntity.rates.isItemSelected.value = input.selectedPosition==index
            }
